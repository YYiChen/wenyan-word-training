...
- Pending is the lowest priority: a pending side automatically adopts the
  other side's manual conclusion.
- Equal non-pending conclusions are not a conflict.
- Different non-pending conclusions are a blocking review conflict that
  requires an explicit human resolution (local / incoming / skip).
- Ordinary wenyan-question-import packages never carry a trustworthy
  teacher review.
"""
...
def classroom_bank():
    return make_bank("bank_classroom", [
        ("A", "pending", ""),
        ("B", "passed", "教室已审"),
        ("C", "needs_revision", "教室待改"),
        ("D", "skipped", "教室跳过"),
        ("E", "passed", "教室已审"),
        ("F", "needs_revision", "教室待改"),
    ], id_prefix="q")
...
def teacher_bank(*, same_bank=False):
    ...
    return make_bank(bank_id, [
        ("A", "passed", "老师已审"),
        ("B", "pending", ""),
        ("C", "needs_revision", "老师待改"),
        ("D", "skipped", "老师跳过"),
        ("E", "needs_revision", "老师待改"),
        ("F", "skipped", "老师跳过"),
...
def conflict_ids(report):
    return [item["conflictId"] for item in report["reviewConflicts"]]


class CrossBankMergeTests(unittest.TestCase):
    def test_acceptance_scenario_auto_and_conflicts(self) -> None:
        ...
        self.assertEqual(review_of(merged, "B")["status"], "passed")
        self.assertEqual(review_of(merged, "C")["status"], "needs_revision")
        self.assertEqual(review_of(merged, "D")["status"], "skipped")
        # E (passed vs needs_revision) and F (needs_revision vs skipped)
        # are blocking conflicts; the preview bank keeps local as placeholder.
        self.assertEqual(len(outcome["report"]["reviewConflicts"]), 2)
        self.assertEqual(review_of(merged, "E")["status"], "passed")
        self.assertEqual(review_of(merged, "F")["status"], "needs_revision")
    ...
    def test_acceptance_scenario_resolutions(self) -> None:
        classroom = classroom_bank()
        teacher = teacher_bank()
        preview_outcome = merge_question_bank_v4(classroom, teacher, mode="merge")
        ids = conflict_ids(preview_outcome["report"])
        self.assertEqual(len(ids), 2)
        by_local = {item["localQuestionId"]: item for item in preview_outcome["report"]["reviewConflicts"]}
        resolutions = {
            by_local["qe"]["conflictId"]: "incoming",
            by_local["qf"]["conflictId"]: "skip",
        }
        merged = merge_question_bank_v4(
            classroom, teacher, mode="merge", review_resolutions=resolutions
        )["bank"]
        self.assertEqual(review_of(merged, "A")["status"], "passed")
        self.assertEqual(review_of(merged, "B")["status"], "passed")
        self.assertEqual(review_of(merged, "C")["status"], "needs_revision")
        self.assertEqual(review_of(merged, "D")["status"], "skipped")
        self.assertEqual(review_of(merged, "E")["status"], "needs_revision")
        self.assertEqual(review_of(merged, "F")["status"], "needs_revision")

    def test_same_bank_acceptance_scenario(self) -> None:
        classroom = classroom_bank()
        teacher = teacher_bank(same_bank=True)
        outcome = merge_question_bank_v4(classroom, teacher, mode="merge")
        self.assertEqual(len(outcome["report"]["reviewConflicts"]), 2)
        by_local = {item["localQuestionId"]: item for item in outcome["report"]["reviewConflicts"]}
        resolutions = {
            by_local["qe"]["conflictId"]: "incoming",
            by_local["qf"]["conflictId"]: "local",
        }
        merged = merge_question_bank_v4(
            classroom, teacher, mode="merge", review_resolutions=resolutions
        )["bank"]
    ...
    def test_apply_without_resolutions_is_blocked(self) -> None:
        classroom = classroom_bank()
        teacher = teacher_bank()
        with self.assertRaises(UnresolvedReviewConflicts):
            merge_question_bank_v4(classroom, teacher, mode="merge", review_resolutions={})
        # A partial map is also rejected.
        preview_outcome = merge_question_bank_v4(classroom, teacher, mode="merge")
        one_id = conflict_ids(preview_outcome["report"])[:1]
        with self.assertRaises(UnresolvedReviewConflicts):
            merge_question_bank_v4(
                classroom, teacher, mode="merge",
                review_resolutions={one_id[0]: "local"},
            )
        # Unknown IDs and bad values are rejected without touching the bank.
        with self.assertRaises(ValueError):
            merge_question_bank_v4(
                classroom, teacher, mode="merge",
                review_resolutions={"review-conflict-unknown": "local", **{i: "local" for i in one_id}},
            )
        with self.assertRaises(ValueError):
            merge_question_bank_v4(
                classroom, teacher, mode="merge",
                review_resolutions={i: "sideways" for i in conflict_ids(preview_outcome["report"])},
            )

    def test_use_imported_never_decides_review_conflict(self) -> None:
        classroom = classroom_bank()
        teacher = teacher_bank()
        preview_outcome = merge_question_bank_v4(
            classroom, teacher, mode="merge", strategy="use_imported"
        )
        # use_imported does not resolve review conflicts automatically.
        self.assertEqual(len(preview_outcome["report"]["reviewConflicts"]), 2)

    def test_conflict_ids_are_deterministic(self) -> None:
        classroom = classroom_bank()
        teacher = teacher_bank()
        first = conflict_ids(merge_question_bank_v4(classroom, teacher, mode="merge")["report"])
        second = conflict_ids(merge_question_bank_v4(classroom, teacher, mode="merge")["report"])
        self.assertEqual(first, second)
        self.assertTrue(all(item.startswith("review-conflict-") for item in first))
        # The ID binds both sides and the fingerprint.
        self.assertEqual(len(set(first)), 2)

    def test_conflict_payload_contract(self) -> None:
        classroom = classroom_bank()
        teacher = teacher_bank()
        preview = build_import_preview(classroom, teacher, mode="merge")
        entries = [item for item in preview["conflicts"] if item.get("kind") == "review"]
        self.assertEqual(len(entries), 2)
        for entry in entries:
            self.assertIn("conflictId", entry)
            self.assertIn("localQuestionId", entry)
            self.assertIn("incomingQuestionId", entry)
            self.assertIn("questionDisplay", entry)
            self.assertIn("localReview", entry)
            self.assertIn("incomingReview", entry)
            display = entry["questionDisplay"]
            self.assertTrue(display["word"])
            self.assertTrue(display["sentence"])
            self.assertTrue(display["article"])

    def test_different_bank_conflict_keeps_local_id(self) -> None:
        classroom = make_bank("bank_classroom", [("A", "passed", "教室已审")], id_prefix="q")
        teacher = make_bank(
            "bank_teacher", [("A", "needs_revision", "老师待改")],
            article_id="teacher_article_9", book_id="teacher_book_9", id_prefix="tq",
        )
        outcome = merge_question_bank_v4(classroom, teacher, mode="merge")
        self.assertEqual(len(outcome["bank"]["questions"]), 1)
        self.assertEqual(len(outcome["report"]["reviewConflicts"]), 1)
        entry = outcome["report"]["reviewConflicts"][0]
        self.assertEqual(entry["localQuestionId"], "qa")
        self.assertEqual(entry["incomingQuestionId"], "tqa")
        merged = merge_question_bank_v4(
            classroom, teacher, mode="merge",
            review_resolutions={entry["conflictId"]: "incoming"},
        )["bank"]
        self.assertEqual([q["id"] for q in merged["questions"]], ["qa"])
        self.assertEqual(merged["workflow"]["reviews"]["qa"]["status"], "needs_revision")
        self.assertEqual(merged["workflow"]["reviews"]["qa"]["note"], "老师待改")

    def test_different_bank_pending_supplement(self) -> None:
        ...
        self.assertEqual(outcome["report"]["reviewConflicts"], [])

    def test_new_full_bank_question_travels_with_review(self) -> None:
        classroom = make_bank("bank_classroom", [("A", "passed", "教室已审")], id_prefix="q")
        teacher = make_bank(
            "bank_teacher", [("A", "passed", "老师已审"), ("F", "passed", "老师新增已审")],
            article_id="teacher_article_1", book_id="teacher_book_1", id_prefix="tq",
        )
        merged = merge_question_bank_v4(classroom, teacher, mode="merge")["bank"]
        newcomers = [q for q in merged["questions"] if q["id"] not in {"qa"}]
    ...
    def test_content_conflict_never_enters_review_queue(self) -> None:
        classroom = classroom_bank()
        teacher = copy.deepcopy(classroom)
        teacher["questions"][0]["explanation"] = "利：锋利，形容词。"
        teacher["workflow"]["reviews"]["qa"] = {"status": "needs_revision"}

        preserved = merge_question_bank_v4(classroom, teacher, mode="merge", strategy="preserve_local")
        self.assertEqual(preserved["bank"]["questions"][0]["explanation"], "利：锋利。")
        self.assertEqual(preserved["bank"]["workflow"]["reviews"]["qa"]["status"], "pending")
        self.assertEqual(preserved["report"]["reviewConflicts"], [])

        adopted = merge_question_bank_v4(classroom, teacher, mode="merge", strategy="use_imported")
        self.assertEqual(adopted["bank"]["questions"][0]["explanation"], "利：锋利，形容词。")
        self.assertEqual(adopted["bank"]["workflow"]["reviews"]["qa"]["status"], "needs_revision")
        self.assertEqual(adopted["report"]["reviewConflicts"], [])
    ...
    def test_full_bank_replace_preserves_imported_workflow(self) -> None:
        ...
        self.assertEqual(replaced["workflow"]["reviews"][by_word["利"]]["status"], "passed")
        self.assertEqual(replaced["workflow"]["reviews"][by_word["信"]]["status"], "needs_revision")
        self.assertEqual(replaced["workflow"]["reviews"][by_word["任"]]["status"], "skipped")
    ...
    def test_preview_stats_match_apply(self) -> None:
        ...
        # A supplemented; C/D same; B both pending... (B: passed vs pending
        # keeps local, no supplement); E/F blocking conflicts.
        self.assertEqual(summary["reviewsSupplemented"], 1)
        self.assertEqual(summary["sameReviewed"], 2)
        self.assertEqual(summary["bothPending"], 0)
        self.assertEqual(summary["reviewConflicts"], 2)
        self.assertEqual(summary["importedReviewedNewQuestions"], 0)
    ...
    def test_conflict_resolution_history_and_revoke(self) -> None:
        classroom = classroom_bank()
        teacher = teacher_bank()
        preview_outcome = merge_question_bank_v4(classroom, teacher, mode="merge")
        by_local = {item["localQuestionId"]: item for item in preview_outcome["report"]["reviewConflicts"]}
        ...
        # Only actually changed reviews are recorded: E supplemented, A
        # supplemented; F kept local produces no delta.
        self.assertEqual(set(delta["updatedReviews"]), {"qa", "qe"})
        self.assertEqual(delta["updatedReviews"]["qe"]["before"]["status"], "passed")
        self.assertEqual(delta["updatedReviews"]["qe"]["after"]["status"], "needs_revision")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertEqual(revoked_bank["workflow"]["reviews"]["qe"]["status"], "passed")
                self.assertEqual(revoked_bank["workflow"]["reviews"]["qa"]["status"], "pending")
                ...
                edited["workflow"]["reviews"]["qe"] = {
                    "status": "skipped", "note": "教室复改",
