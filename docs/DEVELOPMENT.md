...
```powershell
python tools\build_release.py --output .\release-build-v1.4.10-check
```

只构建源码包或 GitHub Windows 更新包：

```powershell
python tools\build_release.py --output .\release-build-v1.4.10-source --source-only
python tools\build_release.py --output .\release-build-v1.4.10-github --github-only
```
