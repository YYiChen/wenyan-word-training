...
GitHub 仓库、源码包和 Windows release 包均不包含真实题库、审查记录或导入历史。首次运行时会在应用旁边创建空白本地数据，教师可在管理员后台导入自己的题库；排行榜、答题记录和管理员配置仍保存在用户数据目录中。完整答题记录只在管理员后台查看，学生端只提交本局结果并查看排行榜。

学生结果通过本机服务的幂等接口保存；排行榜成绩会记录教材/篇目范围、答题时长和计分规则快照，同分时先提交者优先。答题记录按折叠状态分别保留最近一个月各 100 条，自动备份保留最近 100 份且不超过 90 天。
