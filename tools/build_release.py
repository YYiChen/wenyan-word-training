...
GitHub 仓库中的 `public-data/questions.json` 只是一份公开示例题库，每篇文章保留 1-3 道已复核题目；本发布包不会携带这份示例题库，也不包含完整教师题库、审查记录或导入历史。首次运行时会在应用旁边创建空白本地数据，教师可在管理员后台导入自己的题库；排行榜、答题记录和管理员配置仍保存在用户数据目录中。启动窗口只负责本地服务生命周期，学生答题页和管理员后台仍在默认浏览器中打开。
